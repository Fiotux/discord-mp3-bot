...
function voiceInit1() {
  bot.channels.fetch(config.voiceChannel1).then(channel => {
...
function voiceInit2() {
  bot.channels.fetch(config.voiceChannel2).then(channel => {
    const connection = joinVoiceChannel({
      channelId: channel.id,
      guildId: channel.guild.id,
      adapterCreator: channel.guild.voiceAdapterCreator
...
function voiceInit3() {
  bot.channels.fetch(config.voiceChannel3).then(channel => {
    const connection = joinVoiceChannel({
      channelId: channel.id,
      guildId: channel.guild.id,
      adapterCreator: channel.guild.voiceAdapterCreator
...
function playAudio() {
  ...
  let audioname = audio.replace(".mp3", "")

  console.log('Now playing: ' + audioname);
  if (txtFile === true) {
  ...
  const statusEmbed = new Discord.MessageEmbed()
      .addField('Now Playing', `${audioname}`)
      .setColor('#0066ff')
...
bot.on('ready', () => {
  ...
  console.log(`Voice Channel1: ${config.voiceChannel1}`);
  console.log(`Voice Channel2: ${config.voiceChannel2}`);
  console.log(`Voice Channel3: ${config.voiceChannel3}`);
